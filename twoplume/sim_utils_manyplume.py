...
def gen_puff_dict(puff_number, tidx, source_position=(0,0), source_id=0):
    return {
...
def grow_puffs(birth_rate, puff_t, tidx, rng=None, source_position=(0,0), source_id=0):
    if rng is None:
    ...
    new_rows = [ gen_puff_dict(puff_number+i, tidx, source_position, source_id) for i in range(num_births)]    
...
def manual_integrator(puff_t, wind_t, tidx,
    ...
    # Expanded boundaries to accommodate new plumes
    puff_t = puff_t.query("(radius > 0) & (x<30) & (y<15) & (x>-5) & (y>-15)")

    # Grow plume
    puff_t = grow_puffs(birth_rate, puff_t, tidx, rng, source_position, source_id)
    
    return puff_t


def get_puffs_df_vector(wind_df, wind_y_var, birth_rate, source_position=(0,0), source_id=0, verbose=True):
    """Fast vectorized euler stepper for single plume"""
    # Initialize
    n_steps = int((wind_df['time'].max() - wind_df['time'].min())*100)
    tidx = 0
    puff_t = pd.DataFrame([gen_puff_dict(puff_number=0, tidx=tidx, source_position=source_position, source_id=source_id)])
    ...
    for i in tqdm.tqdm(range(n_steps)):
        puff_t = manual_integrator(puff_t, wind_t, tidx, 
            ...
            source_position=source_position, source_id=source_id)
