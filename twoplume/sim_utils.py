...
def gen_puff_dict(puff_number, tidx, source_position=(0,0)):
    return {
...
def grow_puffs(birth_rate, puff_t, tidx, rng=None, source_position=(0,0)):
    if rng is None:
        rng = np.random.RandomState()
    num_births = sp.stats.poisson.rvs(birth_rate, size=1, random_state=rng)[0]
    puff_number = puff_t['puff_number'].max() + 1 if len(puff_t) > 0 else 0
    
    new_rows = [ gen_puff_dict(puff_number+i, tidx, source_position) for i in range(num_births)]    
...
def manual_integrator(puff_t, wind_t, tidx,
    ...
    puff_t['y'] += wind_t['wind_y'].item()*dt + rng.normal(0, wind_y_var, size=n_puffs)*dt
    ...
    # Grow plume
    puff_t = grow_puffs(birth_rate, puff_t, tidx, rng, source_position)
    
    return puff_t

def get_puffs_df_vector(wind_df, wind_y_var, birth_rate, source_position=(0,0), verbose=True):
    """Fast vectorized euler stepper for single plume"""
    print(wind_df.shape, wind_y_var, birth_rate)
    # Initialize
    n_steps = int((wind_df['time'].max() - wind_df['time'].min())*100)
    tidx = 0
    puff_t = pd.DataFrame([gen_puff_dict(puff_number=0, tidx=tidx, source_position=source_position)])
    ...
    for i in tqdm.tqdm(range(n_steps)):
        puff_t = manual_integrator(puff_t, wind_t, tidx, 
            birth_rate=birth_rate, wind_y_var=wind_y_var, rng=rng,
...
def get_dual_puffs_df_vector(wind_df, wind_y_var, birth_rate, source_positions, verbose=True):
    """Fast vectorized euler stepper for multiple plumes"""
    print(f"Generating {len(source_positions)} plumes from sources: {source_positions}")
    ...
    # Create separate RNGs for each plume
    rngs = [np.random.RandomState(i) for i in range(len(source_positions))]
    
    # Initialize each plume
    plume_states = []
    for i, source_pos in enumerate(source_positions):
        plume_states.append({
            'puff_t': pd.DataFrame([gen_puff_dict(puff_number=0, tidx=tidx, source_position=source_pos)]),
            'rng': rngs[i],
    ...
    for i in tqdm.tqdm(range(n_steps)):
        # Update each plume
        for plume in plume_states:
            plume['puff_t'] = manual_integrator(
                plume['puff_t'], wind_t, tidx,
                birth_rate=birth_rate, 
                wind_y_var=wind_y_var,
                rng=plume['rng'],
                source_position=plume['source_pos']
            )
            puff_dfs.append(plume['puff_t'].copy())
