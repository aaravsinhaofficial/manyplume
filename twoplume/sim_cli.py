...
parser.add_argument('--duration', metavar='d', type=int, default=200,
                    help='simulation duration in seconds')
parser.add_argument('--cores', metavar='c', type=int, default=8,
                    help='number of cores to use')
parser.add_argument('--dataset_name', type=str, default='twoplume')
parser.add_argument('--fname_suffix', type=str, default='')
parser.add_argument('--dt', type=float, default=0.01,
                    help='time per step (seconds)')
parser.add_argument('--wind_magnitude', type=float, default=0.5,
                    help='m/s')
parser.add_argument('--wind_y_varx', type=float, default=1.0)
parser.add_argument('--birth_rate', type=float, default=1.5,
                    help='Poisson birth_rate parameter')
parser.add_argument('--outdir', type=str, default=config.datadir)
parser.add_argument('--source_positions', type=str, default='-1,-1;-1,1',  # Updated default
                    help='Semicolon-separated list of x,y plume source positions (e.g. "0,-1;0,1")')
parser.add_argument('--warmup_time', type=float, default=0.0,
                    help='Warm-up time in seconds before plume is fully developed')  # ADDED

args = parser.parse_args()
print(args)

# ADDED: Calculate total duration with warm-up
...
wind_df = sim_utils.get_wind_xyt(
    total_duration,  # MODIFIED: Use total duration
    dt=args.dt,
...
if multi_plume:
    ...
    puff_df = sim_utils.get_dual_puffs_df_vector(
        wind_df,
    ...
    print(f"Generating single plume from source: {source_positions[0]}")
