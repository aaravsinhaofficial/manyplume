...
class PlumeEnvironment(gym.Env):
    ...
    def __init__(self, 
        ...
        env_dt=0.04,
        loc_algo='quantile',
        qvar=0.95, # Variance of init. location; higher = more off-plume initializations
        time_algo='uniform',
        angle_algo='uniform',
        ...
        r_shaping=['step', 'oob','end'], # 'step', 'end'
        ...
        self.fixed_x = 2.0 
        self.fixed_y = -1.0 # Start at (2, -1) as requested

        # Define actual source positions
        self.source_positions = [(-1, -1), (-1, 1)]
        ...
        self.initial_min_dist = None  # Will store initial min distance to sources
    ...
    def get_initial_location(self, algo):
        ...
        if 'fixed' in algo:
            loc_xy = np.array( [self.fixed_x, self.fixed_y] )  # Fixed start at (2, -1)
    ...
    def reset(self):
        ...
        max_offset_steps = int(self.reset_offset_tmax * self.fps)
        rand_int = int.from_bytes(os.urandom(4), 'little')
        self.step_offset = rand_int % (max_offset_steps + 1)
        N = len(self.t_vals)
        M = self.episode_steps_max
        idx = self.episode_step + self.step_offset
        print(f"[DEBUG] len(t_vals)={N}, episode_steps_max={M}, step_offset={self.step_offset}, idx={idx}")
        if idx >= N:
            print(f"[ERROR] Index {idx} out of bounds for t_vals of length {N}. Adjusting step_offset.")
            self.step_offset = max(0, N - M - 1)  # Ensure we don't go out of bounds
            idx = self.episode_step + self.step_offset
        self.t_val = self.t_vals[self.episode_step + self.step_offset] 
        ...
        # Initialize agent to fixed location (2, -1)
        self.agent_location = self.get_initial_location(self.loc_algo)
        self.agent_location_last = self.agent_location
        self.agent_location_init = self.agent_location

        # Calculate initial min distance to sources
        self.initial_min_dist = min(
            np.linalg.norm(np.array(self.agent_location) - np.array(src))
    ...
    def step(self, action):
        ...
        # Calculate distance to both sources and find the nearest
        prev_dists = [np.linalg.norm(np.array(self.agent_location_last) - np.array(src)) 
                     for src in self.source_positions]
        prev_min_dist = min(prev_dists)

        current_dists = [np.linalg.norm(np.array(self.agent_location) - np.array(src)) 
                        for src in self.source_positions]
        current_min_dist = min(current_dists)
        
        ### ----------------- End conditions / Is the trial over ----------------- ### 
        # Update home condition to check if agent reached either source
        is_home = current_min_dist <= self.homed_radius 
        ...
        ### ----------------- REWARD LOGIC ----------------- ### 
        # Basic reward - larger reward for reaching source
        reward = self.rewards['homed'] if is_home else self.rewards['tick']
        if observation[2] <= config.env['odor_threshold']: 
            reward += 5 * self.rewards['tick']

        # Reward shaping: step
        r_radial_step = 0
        if 'step' in self.r_shaping:
            # Calculate progress toward nearest source
            r_radial_step = 5 * (prev_min_dist - current_min_dist)   # positive when getting closer to source
            
            # Only penalize moving away when off plume
            if observation[2] <= config.env['odor_threshold']:
                r_radial_step = min(0, r_radial_step)   
                
            # Multiplier for overshooting source (if agent is in negative x)
            if 'overshoot' in self.r_shaping and self.agent_location[0] < 0:
                r_radial_step *= 2

            # Add reward for reducing stray distance from plume
        ...
        # End reward shaping
        radial_distance_reward = 0
        if done and 'end' in self.r_shaping:
            # Calculate progress from initial position to nearest source
            radial_distance_decrease = self.initial_min_dist - current_min_dist
            end_reward = radial_distance_decrease - self.stray_distance
            reward += end_reward

        # Location-based reward - closer to source = higher reward
        r_location = 0
        if 'loc' in self.r_shaping:
            r_location = 1/(1 + current_min_dist)   # closer to source = higher reward
            r_location /= (1 + 5 * self.stray_distance)
            reward += r_location 

        # Action penalty terms
        ...
        info = {
            ...
            'min_dist_to_source': current_min_dist,  # Add min distance to source for debugging
            }
        ...
        if done:
            ...
            print(f"[EPISODE] Total reward: {self.episode_reward:.2f}, Min distance to source: {current_min_dist:.2f}")
