...
class PlumeEnvironment(gym.Env):
  ...
  def __init__(self, 
    ...
    if self.walking:
        ...
        self.homed_radius = 0.01 # m i.e. 1cm walk (tighter than before)
        self.stray_max = 0.03 # meters (tighter bounds)
        # Adjust rewards for walking scale and physiology
        self.rewards['tick'] = -1.0/self.episode_steps_max  # Less harsh time penalty
        self.rewards['homed'] = 150.0  # Higher success reward
        
        # Walking-specific reward shaping
        if 'metabolic' not in self.r_shaping:
            self.r_shaping.append('metabolic')
        if 'turn' not in self.r_shaping:
            self.r_shaping.append('turn')
        if 'plume_grad' not in self.r_shaping:
            self.r_shaping.append('plume_grad')
  ...
  def get_initial_location(self, algo):
    loc_xy = None
    if 'uniform' in algo:
        if self.walking and self.walk_curriculum:
            # Curriculum learning: start close to plume center and gradually increase distance
            plume_center = self.get_plume_center_location()
            max_distance = self.get_walking_curriculum_distance()
            
            # Sample uniformly within curriculum distance
            ...
            loc_xy = plume_center + np.array([offset_x, offset_y])
            
            # Ensure within reasonable bounds
            loc_xy[0] = np.clip(loc_xy[0], 0.02, 0.5)
            loc_xy[1] = np.clip(loc_xy[1], -0.1, 0.1)
            
        elif self.walking:
            # Original walking initialization (closer than flying)
            loc_xy = np.array([
              0.2 + np.random.uniform(-0.1, 0.1), 
              np.random.uniform(-0.05, 0.05)])
        else:
            # Original flying initialization
            loc_xy = np.array([
    ...
    if 'quantile' in algo:
        """ 
        Distance curriculum for walking agents
        """
        if self.walking and self.walk_curriculum:
            # Use plume center approach for walking curriculum
            plume_center = self.get_plume_center_location()
            max_distance = self.get_walking_curriculum_distance()
            
            # Add some noise around plume center
            noise_x = np.random.normal(0, max_distance/3)
            noise_y = np.random.normal(0, max_distance/3)
            
            loc_xy = plume_center + np.array([noise_x, noise_y])
            loc_xy[0] = np.clip(loc_xy[0], 0.02, 0.5)
            loc_xy[1] = np.clip(loc_xy[1], -0.1, 0.1)
        else:
            # Original quantile algorithm
  ...
  def step(self, action):
    ...
    # Walking-specific success bonus
    if is_home and self.walking:
        reward += self.walk_success_bonus
    
    if observation[2] <= config.env['odor_threshold'] : # if off plume, more tick penalty
        if self.walking:
            reward += 1.5*self.rewards['tick']  # Gentler penalty for walking
        else:
    ...
    if is_outofbounds and 'oob' in self.r_shaping:
        if self.walking:
            # Gentler OOB penalty for walking with curriculum consideration
            oob_penalty = 1.0*np.linalg.norm(self.agent_location) + 2.0*self.stray_distance
            oob_penalty *= 1.2 if self.agent_location[0] < 0 else 1  
        else:
            # Original OOB penalty for flying
    ...
    if 'step' in self.r_shaping:
        step_multiplier = 3 if self.walking else 5  # Gentler for walking
        r_radial_step = step_multiplier*( np.linalg.norm(self.agent_location_last) - np.linalg.norm(self.agent_location) )
    ...
    # Enhanced walking agent rewards
    r_metabolic = 0
    r_turn_cost = 0
    r_plume_grad = 0
    
    if self.walking:
        # Improved metabolic cost
        if 'metabolic' in self.r_shaping:
            delta_move = np.linalg.norm(np.array(self.agent_location) - np.array(self.agent_location_last))
            r_metabolic = -5.0 * delta_move  # Reduced penalty
            reward += r_metabolic
        
        # Turn cost
        if 'turn' in self.r_shaping:
            turn_magnitude = np.abs(2*(turn_action - 0.5))
            r_turn_cost = -1.0 * turn_magnitude  # Reduced penalty
            reward += r_turn_cost
        
        # Plume gradient following reward
        if 'plume_grad' in self.r_shaping and self.walk_plume_following:
            r_plume_grad = self.get_plume_gradient_reward()
            reward += r_plume_grad
        
        # Curriculum completion bonus
        if 'curriculum' in self.r_shaping:
            curriculum_progress = min(1.0, self.walk_training_step / self.walk_curriculum_steps)
            if is_home and curriculum_progress > 0.8:  # Advanced curriculum stage
                reward += 25.0 * curriculum_progress
    ...
    info = {
        ...
        'r_turn_cost': r_turn_cost,
        'r_plume_grad': r_plume_grad,
        'movex': self.movex,
        'done': done_reason if done else None,
        'radiusx': self.radiusx,
        }
    
    # Walking-specific info
    if self.walking:
        info.update({
            'walk_curriculum_step': self.walk_training_step,
            'walk_curriculum_distance': self.get_walking_curriculum_distance(),
            'plume_center': self.get_plume_center_location().tolist() if hasattr(self, 'data_puffs') else [0, 0],
        })
