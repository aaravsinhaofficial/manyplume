...
def get_selected_df(
    ...
    # Hardcoded paths for your specific model
    ...
    print(f"Loading episodes from: {log_fname}")
    
    with open(log_fname, 'rb') as f_handle:
        episode_logs = pickle.load(f_handle)
    
    print(f"Loaded {len(episode_logs)} episodes")
    
    # Build episodes dataframe
    episodes_df = []
    ...
    print(episodes_df['outcome'].value_counts())
    
    # Filter by minimum episode length
    episodes_df = episodes_df.query("ep_length >= @min_ep_steps")
    print(f"\nAfter filtering (min_ep_steps={min_ep_steps}): {len(episodes_df)} episodes")
    
    # Select episodes by outcome
    if oob_only:
        home_episodes = episodes_df.query('outcome == "HOME"').head(n_episodes_home)
        oob_episodes = episodes_df.query('outcome == "OOB"').head(n_episodes_other)
        selected_df = pd.concat([home_episodes, oob_episodes]).reset_index(drop=True)
    else:
        home_episodes = episodes_df.query('outcome == "HOME"').head(n_episodes_home)
        other_episodes = episodes_df.query('outcome != "HOME"').head(n_episodes_other)
        selected_df = pd.concat([home_episodes, other_episodes]).reset_index(drop=True)
    
    print(f"\nSelected episodes before balancing:")
    print(selected_df['outcome'].value_counts())
    
    # Balance the selection if requested
    if balanced:
        counts = selected_df['outcome'].value_counts()
        min_count = counts.min()
        
        balanced_df = selected_df.groupby('outcome').head(min_count)
        selected_df = balanced_df.reset_index(drop=True)
        
        print(f"\nAfter balancing (min_count={min_count}):")
        print(selected_df['outcome'].value_counts())
