...
class PlumeEnvironment(gym.Env):
  ...
  def __init__(self, 
    ...
    birthx_max=10.0, # overall odor puff birth rate sparsity max
    env_dt=0.04,
    loc_algo='uniform',
    qvar=1.1, # Variance of init. location; higher = more off-plume initializations
    time_algo='uniform',
    angle_algo='uniform',
    ...
    if self.walking:
        ...
        self.homed_radius = 0.022 # m i.e. 18cm walk from 0.20m (flying "homed" distance)
        self.stray_max = 0.05 # meters
        self.rewards['tick'] = -1.2/self.episode_steps_max
        #get agent to actually move towards the source and not just stay in place waiting for time to run out!
  ...
  def get_initial_location(self, algo):
    ...
    if 'uniform' in algo:
        loc_xy = np.array([
            2 + np.random.uniform(-1.3, 1.6), 
            np.random.uniform(-0.75, 0.75)])

        if self.walking:
            loc_xy = np.array([
              0.2 + np.random.uniform(-0.13, 0.13), 
              np.random.uniform(-0.09, 0.09)])

    if 'linear' in algo:
        # TODO
        loc_xy = np.array([
            2 + np.random.uniform(-1, 1.4), 
